...
export default function AppLayout({ children }: { children: React.ReactNode }) {
    ...
    <div className="flex flex-col h-full bg-white text-slate-900 border-r border-slate-200">
      <div className="p-6">
        <div className="flex items-center gap-3 mb-8">
          ...
          <span className="font-bold text-lg leading-tight">Hardware<br/>Inventory Pro</span>
        </div>
  ...
  const getPageTitle = () => {
    const item = mainNavItems.find(i => location === i.href || (i.href !== '/' && location.startsWith(`${i.href}/`)));
  ...
  return (
    <div className="min-h-[100dvh] bg-slate-50 flex flex-col md:flex-row">
      ...
      <div className="flex-1 md:pl-64 flex flex-col min-h-[100dvh]">
          ...
          <div className="flex items-center gap-4">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" className="hidden sm:flex items-center gap-2 border-slate-200 text-slate-700 bg-slate-50">
              ...
              <DropdownMenuContent align="end" className="w-56">
                {stores.map((store) => (
                  <DropdownMenuItem 
                    ...
                    className={cn(currentStore.id === store.id && "bg-blue-50 text-blue-700 font-medium")}
                  >
                    {store.branch_name}
                  </DropdownMenuItem>
                ))}
        ...
        <div className="md:hidden fixed bottom-0 inset-x-0 bg-white border-t border-slate-200 flex justify-around items-center h-16 px-2 pb-safe z-40">
          ...
          <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
            <SheetTrigger asChild>
              <button className="flex flex-col items-center justify-center w-full h-full space-y-1 text-slate-500 hover:text-slate-900">
